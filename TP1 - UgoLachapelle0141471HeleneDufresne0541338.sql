...
-- NOM : Ugo Lachapelle 0141471 - Hélène Dufresne 0541338
...
FROM cours
WHERE date_entrainement BETWEEN '2025-03-02' AND '2025-03-08'
...
SELECT entraineurs.nom, entraineurs.prenom, MIN(abonnements.date_debut) AS minimum, MAX(abonnements.date_debut) AS maximum
FROM entraineurs
...
SELECT membres.nom, membres.prenom, COUNT(reservations.fk_cours) AS decompte
FROM membres
...
SELECT membres.*, SUM(abonnements.prix) AS total
FROM membres
JOIN abonnements ON abonnements.fk_membre = membres.id
GROUP BY membres.id
ORDER BY total DESC
...
SELECT cours.date_entrainement, cours.horaire, entraineurs.nom, entraineurs.prenom, (SELECT GROUP_CONCAT(membres.prenom,' ', membres.nom)
	FROM membres 
	JOIN reservations ON membres.id = reservations.fk_membre
	WHERE reservations.fk_cours = cours.id) AS participants
FROM cours
...
SELECT membres.id, membres.nom, membres.prenom, abonnements.fk_membre, SUM(DATEDIFF(abonnements.date_fin, abonnements.date_debut)) AS jours
FROM membres
JOIN abonnements ON membres.id = abonnements.fk_membre
GROUP BY membres.id
HAVING SUM(DATEDIFF(abonnements.date_fin, abonnements.date_debut)) = (SELECT MAX(jours)
	FROM (SELECT SUM(DATEDIFF(abonnements.date_fin, abonnements.date_debut)) AS jours, abonnements.fk_membre 
		FROM abonnements 
		GROUP BY abonnements.fk_membre) AS tableau)
...
JOIN membres ON reservations.fk_membre = membres.id
GROUP BY membres.id
HAVING COUNT(reservations.id) >= (SELECT AVG(decompte)
	FROM (SELECT COUNT(reservations.id) AS decompte
		FROM reservations
		JOIN membres ON reservations.fk_membre = membres.id
		GROUP BY membres.id) AS tableau1)
...
WHERE fk_membre = (SELECT membres.id
FROM reservations
LEFT JOIN membres ON membres.id = reservations.fk_membre
LEFT JOIN cours ON cours.id = reservations.fk_cours
WHERE membres.nom = 'Caron' AND membres.prenom = 'Jérôme' AND cours.nom = 'Nutrition Sportive' AND cours.date_entrainement = '2025-03-15') 
    AND fk_cours = (SELECT cours.id FROM cours WHERE cours.nom = 'Nutrition Sportive' AND cours.date_entrainement = '2025-03-15')
...
INSERT entraineurs (entraineurs.prenom, entraineurs.nom, entraineurs.specialite, entraineurs.adresse, entraineurs.telephone, entraineurs.courriel)
